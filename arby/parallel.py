...
@ray.remote
def _sq_proj_errors(diff_training,
                    proj_vector,
                    basis_element,
                    dot_product):
...
def reduced_basis(
    ...
    # prepare first parallelization
    
    ray.init()
    CORES_NUM = 4
    SLICE = round(Ntrain / (CORES_NUM - 1))
    results_ids = [
        _sq_proj_errors.remote(
            training_set[i * SLICE: (i + 1) * SLICE, :],
            proj_matrix[0, i * SLICE: (i + 1) * SLICE],
            basis_data[0],
            integration.dot
            )
    ...
    errs = np.hstack([
        pair[0] for pair in results
    ])

    diff_training = np.vstack([
        pair[1] for pair in results
    ])
    ...
    while sigma > greedy_tol:
        ...
        results_ids = [
            _sq_proj_errors.remote(
            diff_training[i * SLICE: (i + 1) * SLICE, :],
            proj_vector[i * SLICE: (i + 1) * SLICE],
            basis_data[nn],
            integration.dot
            )
        for i in range(CORES_NUM)
